...
def get_label_img(paths, img_d, img_rows, img_cols):
    """
    参数：
        paths：要读取的图片路径列表
        img_rows:图片行
    ...
    for path in paths:
        data = np.load(path)
        # Reduce size
        resized = np.reshape(data, (img_d, img_cols, img_rows, 1))
        resized = resized.astype('float32')
        resized /= 255
        datas.append(resized)
    datas = np.array(datas)
...
def get_train_batch(train, label, batch_size, img_d, img_w, img_h):
    ...
    while 1:
        for i in range(0, len(train), batch_size):
            ...
            y = get_label_img(label[i:i+batch_size], img_d, img_w, img_h)
