...
for dir_num in range(num):
    ...
    train_64_input_path = "./vnet_" + str(depth) + "_3_input/train"
    label_64_input_path = "./vnet_" + str(depth) + "_3_input/label"
    ...
    for i in range(len(train_pngs)):
        ...
        label_npy = np.ndarray((depth, image_size, image_size, 1), dtype=np.uint8)
        if (i + depth-1) < len(train_pngs):
            for j in range(depth):
                ...
                label_img_path = os.path.join(label_dir_path, train_pngs[index])
