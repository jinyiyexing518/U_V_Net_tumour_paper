...
data_train_path = "/root/9.V-Net-2.5D-master/v_net/vnet_3_1_input-2/train"
data_label_path = "/root/9.V-Net-2.5D-master/v_net/vnet_3_1_input-2/label"
...
class myVnet(object):
    def __init__(self, img_depth=3, img_rows=512, img_cols=512, img_channel=1, drop=0.5):
    ...
    def get_vnet(self):
        ...
        add7 = layers.add([deconv7, res7])
        add7 = Deconvolution3D(64, [1, 3, 3], strides=(1, 2, 2), activation='relu', padding='same',
                                  kernel_initializer='he_normal')(add7)
        #######################################################################################################################
        ...
        add8 = layers.add([add7, res8])
        add8 = Deconvolution3D(32, [1, 3, 3], strides=(1, 2, 2), activation='relu', padding='same',
                                  kernel_initializer='he_normal')(add8)
        #######################################################################################################################
    ...
    def train(self):
        ...
        #加载已经训练好的模型
        losses = WeightedBinaryCrossEntropy()
        model = load_model('./model_pre_mean/vnet_tumour_3_1_epoch10-fpn.hdf5', custom_objects={'dice_coef': dice_coef,'dice_coef_loss': dice_coef_loss,'weighted_binary_crossentropy':losses.weighted_binary_crossentropy})
        print("got vnet")
        print(model.summary())  # 显示相关的网络信息

        # 保存的是模型和权重（参数设置）在每个epoch后保存模型到filepath。
        model_checkpoint = ModelCheckpoint('./model_pre_mean/vnet_tumour_3_1_epoch11-fpn.hdf5', monitor='loss',
                                           verbose=1, save_best_only=True)
        keras.callbacks.EarlyStopping(monitor='val_loss',
                                      patience=0,
                                      verbose=0,
                                      mode='auto')#当监测值不再改善时，该回调函数将中止训练
        print('Fitting model...')

        # （2）创建一个实例history
        history = LossHistory()
        # （3）训练，在callbacks中加入history最后才能绘制收敛曲线
        model.fit_generator(
            generator=get_train_batch(train_path_list, label_path_list, train_batch_size, 3, 512, 512),
            epochs=epoch, verbose=1,
            steps_per_epoch=count // train_batch_size,
            callbacks=[model_checkpoint, history],
            workers=1)
        # （4）绘制acc-loss曲线，每个batch为一个横坐标
        history.loss_plot('batch')
        plt.savefig('./curve_figure_pre_mean/vnet_tumour_dice_loss_curve_3_1_epoch9_little.png')
        from keras.utils import plot_model
        # 因为模型结果不能显示，所以保存为图片！！
        plot_model(model, to_file='model_vnet3_1_FPN.png')
