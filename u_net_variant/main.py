...
import os
from u_net_variant.net.UNet import Unet, resnet34_unet
from u_net_variant.net.attention_unet import AttU_Net
from u_net_variant.net.channel_unet import myChannelUnet
from u_net_variant.net.r2unet import R2U_Net
from u_net_variant.net.segnet import SegNet
from u_net_variant.net.unetpp import NestedUNet
from u_net_variant.net.fcn import get_fcn8s
from u_net_variant.dataset import *
from u_net_variant.metrics import *
from torchvision.transforms import transforms
from u_net_variant.plot import loss_plot
from u_net_variant.plot import metrics_plot
...
def getArgs():
    ...
    parse.add_argument('--dataset', default='liver')
...
def getModel(args):
    ...
    if args.arch == 'fcn8s':
        assert args.dataset != 'esophagus', "fcn8s模型不能用于数据集esophagus，因为esophagus数据集为80x80，" \
                                            "经过5次的2倍降采样后剩下2.5x2.5，分辨率不能为小数，建议把数据集resize成更高的分辨率再用于fcn"
        model = get_fcn8s(1).to(device)
    if args.arch == 'cenet':
        from u_net_variant.net.cenet import CE_Net_
