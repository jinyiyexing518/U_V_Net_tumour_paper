...
data_train_path = "../V_Net25_tumour/vnet_3_1_input/train"
data_label_path = "../V_Net25_tumour/vnet_3_1_input/label"
...
class myVnet(object):
    ...
    def encode_layer(self, kernel_num, kernel_size, input):
        ...
        conv = Conv3D(2*kernel_num, [1, 1, 1], activation='relu', padding='same',
                      kernel_initializer='he_normal')(input)
        # 第一次卷积，层内卷积
        conv1_1 = Conv3D(kernel_num, kernel_size, activation='relu', padding='same',
                         kernel_initializer='he_normal')(input)
        conv1_2 = Conv3D(kernel_num, kernel_size, activation='relu', padding='same',
                         kernel_initializer='he_normal')(input)
        merge_1 = Concatenate(axis=4)([conv1_1, conv1_2])
        conv1 = self.BN_operation(merge_1)
        conv1 = Dropout(self.drop)(conv1)

        # 第二次卷积，层间卷积
        conv2_1 = Conv3D(kernel_num, [3, 1, 1], activation='relu', padding='same',
                         kernel_initializer='he_normal')(conv1)
        conv2_2 = Conv3D(kernel_num, [3, 1, 1], activation='relu', padding='same',
                         kernel_initializer='he_normal')(conv1)
        merge_2 = Concatenate(axis=4)([conv2_1, conv2_2])
        conv2 = self.BN_operation(merge_2)
        conv2 = Dropout(self.drop)(conv2)

        conv3_1 = Conv3D(kernel_num, kernel_size, activation='relu', padding='same',
                         kernel_initializer='he_normal')(conv2)
        conv3_2 = Conv3D(kernel_num, kernel_size, activation='relu', padding='same',
                         kernel_initializer='he_normal')(conv2)
        merge_3 = Concatenate(axis=4)([conv3_1, conv3_2])
        conv3 = self.BN_operation(merge_3)
        conv3 = Dropout(self.drop)(conv3)

        conv4_1 = Conv3D(kernel_num, [3, 1, 1], activation='relu', padding='same',
                         kernel_initializer='he_normal')(conv3)
        conv4_2 = Conv3D(kernel_num, [3, 1, 1], activation='relu', padding='same',
                         kernel_initializer='he_normal')(conv3)
        merge_4 = Concatenate(axis=4)([conv4_1, conv4_2])
        conv4 = self.BN_operation(merge_4)
        conv4 = Dropout(self.drop)(conv4)

        # 残差
        res = layers.add([conv, conv4])
    ...
    def train(self):
        ...
        model = self.get_vnet()
        #        model = load_model('./model_pre_mean/vnet_tumour_3_1_epoch1.hdf5',
        #                           custom_objects={'dice_coef': dice_coef, 'dice_coef_loss': dice_coef_loss})
        print("got vnet")

        # 保存的是模型和权重
        model_checkpoint = ModelCheckpoint('./model_pre_mean/vnet_tumour_3_1_epoch1.hdf5', monitor='loss',
        ...
        plt.savefig('./curve_figure_pre_mean/vnet_tumour_dice_loss_curve_3_1_epoch1.png')
