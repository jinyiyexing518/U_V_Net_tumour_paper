...
data_train_path = "../dataset/data_cv_clip/train"
data_label_path = "../dataset/data_cv_clip/label"
...
#       plt.savefig('./curve_figure/unet_pure_liver_raw_0_129_entropy_dice_curve.png')
       plt.savefig('./curve_figure/unet_tumour_dice2.png')
...
#       plt.savefig('./curve_figure/unet_pure_liver_raw_0_129_entropy_loss_curve.png')
       plt.savefig('./curve_figure/unet_tumour_loss2.png')
       plt.show()
...
#        model.compile(optimizer=Adam(lr=1e-4), loss='binary_crossentropy', metrics=['accuracy',dice_coef])
    ...
    def train(self):
        ...
        model = self.get_unet()
#         model = load_model('./model/unet_tumour1.hdf5', custom_objects={'dice_coef': dice_coef,'dice_coef_loss': dice_coef_loss})
        print("got unet")

        # 保存的是模型和权重
        model_checkpoint = ModelCheckpoint('./model/unet_tumour2.hdf5', monitor='loss',verbose=1, save_best_only=True)
        print('Fitting model...')
