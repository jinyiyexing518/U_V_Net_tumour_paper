...
from u_net.test_model.unet_test import dir_num
# dir_num = 129
# 制作哪一个dir的数据
# dir_num -= 121
image_size = 400
...
train_64_input_path = "./unet_" + str(depth) + "_1_test_npy/train"
label_64_input_path = "./unet_" + str(depth) + "_1_test_npy/label"
...
train_npys = np.ndarray((count_num, image_size, image_size, 1), dtype=np.uint8)
label_npys = np.ndarray((count_num, image_size, image_size, 1), dtype=np.uint8)

for i in range(len(train_pngs)):
    train_npy = np.ndarray((image_size, image_size, 1), dtype=np.uint8)
    ...
    if (i + depth-1) < len(train_pngs):
        for j in range(depth):
            ...
            label_img_path = os.path.join(label_dir_path, label_pngs[index])
