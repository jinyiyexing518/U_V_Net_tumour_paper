...
from refine_net.test_model.refine_net_test import dir_num, model_num, model
# dir_num = 130
# model = "liver"
...
for i in range(len(npy_data)):
    ...
    if (np.nan not in ASSD) and (np.inf not in ASSD) and hausdorff != (np.inf or np.nan)\
            and (np.nan not in surface_overlap) and (np.inf not in surface_overlap):
    ...
    predict_save_name_for_paper = "num_" + str(i + 1) + "_pixel_num_" + str(pixel_num) + "_dice_" \
                                  + str(round(dice, 4)) + "_Refine_Net" + ".png"
