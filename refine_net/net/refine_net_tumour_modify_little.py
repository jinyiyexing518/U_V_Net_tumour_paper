...
data_train_path = "../U_Net_tumour/data_dir_png/train"
data_label_path = "../U_Net_tumour/data_dir_png/label"
...
class myUnet(object):
    ...
    def res_conv_unit(self, kernel_num, input):
        ...
        drops = Dropout(0.5)(conv2)
        res = layers.add([drops, conv1])
    ...
    def get_unet(self):
        ...
        lay1 = self.res_conv_unit(32, inputs)
        pool1 = MaxPooling2D(pool_size=(2, 2))(lay1)
        pool1 = self.BN_operation(pool1)

        lay2 = self.res_conv_unit(64, pool1)
        pool2 = MaxPooling2D(pool_size=(2, 2))(lay2)
        pool2 = self.BN_operation(pool2)

        lay3 = self.res_conv_unit(128, pool2)
        pool3 = MaxPooling2D(pool_size=(2, 2))(lay3)
        pool3 = self.BN_operation(pool3)

        lay4 = self.res_conv_unit(256, pool3)
        pool4 = MaxPooling2D(pool_size=(2, 2))(lay4)
        pool4 = self.BN_operation(pool4)
        # Residual convlution unit
        # 第四层
        res4 = self.res_conv_unit(256, pool4)
        res4 = self.res_conv_unit(256, res4)
        res4 = Conv2D(256, [3, 3], activation='relu', padding='same',
                      kernel_initializer='he_normal')(res4)
        up4 = Conv2D(256, 2, activation='relu', padding='same', kernel_initializer='he_normal')(
            UpSampling2D(size=(2, 2))(res4))
        chained_pool4 = self.chained_residual_pool(256, up4)
        chained_pool4 = self.res_conv_unit(256, chained_pool4)
        # 第三层
        res3 = self.res_conv_unit(128, pool3)
        res3 = self.res_conv_unit(128, res3)
        res3 = Conv2D(128, 3, activation='relu', padding='same',
        ...
        chained_pool4 = Conv2D(128, 3, activation='relu', padding='same',
                               kernel_initializer='he_normal')(chained_pool4)
        chained_pool4 = Conv2D(128, 2, activation='relu', padding='same', kernel_initializer='he_normal')(
                      UpSampling2D(size=(2, 2))(chained_pool4))
        up34 = layers.add([up3, chained_pool4])
        chained_pool3 = self.chained_residual_pool(128, up34)
        chained_pool3 = self.res_conv_unit(128, chained_pool3)
        # 第二层
        res2 = self.res_conv_unit(128, pool2)
        res2 = self.res_conv_unit(128, res2)
        res2 = Conv2D(64, 3, activation='relu', padding='same',
        ...
        chained_pool3 = Conv2D(64, 3, activation='relu', padding='same',
                               kernel_initializer='he_normal')(chained_pool3)
        chained_pool3 = Conv2D(64, 2, activation='relu', padding='same', kernel_initializer='he_normal')(
                      UpSampling2D(size=(2, 2))(chained_pool3))
        up234 = layers.add([up2, chained_pool3])
        chained_pool2 = self.chained_residual_pool(64, up234)
        chained_pool2 = self.res_conv_unit(64, chained_pool2)
        # 第一层
        res1 = self.res_conv_unit(128, pool1)
        res1 = self.res_conv_unit(128, res1)
        res1 = Conv2D(32, 3, activation='relu', padding='same',
                      kernel_initializer='he_normal')(res1)
        up1 = Conv2D(32, 2, activation='relu', padding='same', kernel_initializer='he_normal')(
            UpSampling2D(size=(2, 2))(res1))
        chained_pool2 = Conv2D(32, 3, activation='relu', padding='same',
                               kernel_initializer='he_normal')(chained_pool2)
        chained_pool2 = Conv2D(32, 2, activation='relu', padding='same', kernel_initializer='he_normal')(
                      UpSampling2D(size=(2, 2))(chained_pool2))
        up1234 = layers.add([up1, chained_pool2])
        chained_pool1 = self.chained_residual_pool(32, up1234)
        chained_pool1 = self.res_conv_unit(32, chained_pool1)

        conv9 = Conv2D(16, 3, activation='relu', padding='same', kernel_initializer='he_normal')(chained_pool1)
