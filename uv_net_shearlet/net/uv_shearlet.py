...
from uv_net_shearlet.net.fit_generator import get_path_list, get_train_batch
import matplotlib.pyplot as plt

net_name = "uvnet_shearlet"
...
def Shearlet_Transform_Fun(image):
    """
        功能：实现shearlet变换
        输入：灰度图像
        输出：33channel高频细节图 + shearlet变换规则
        所需库文件：pip install https://github.com/stefanloock/pyshearlab/archive/master.zip
    """
    print("loading image...")
    scales = 3
    ...
    # decomposition
    coeffs = np.real(pyshearlab.SLsheardec2D(X, shearletSystem))

    return coeffs, shearletSystem


def Shearlet_Restruction_Fun(coeffs, shearletSystem):
    # restruction
...
class myUVnet(object):
    ...
    def get_uvnet(self):
        ...
        inputs_numpy = K.eval(inputs)
        coffes, shearlet_system = Shearlet_Transform_Fun(inputs_numpy)
